...
const viewDetail = (id) => {
    router.visit("/daftararsip-detail", { method: "get", data: { id: id } });
};
...
const fetchData = () => {
    axios
        .get(
            `/api/daftararsip?page=${currentPage}&perPage=${perPage.value}&sortField=${sortField.value}&sortOrder=${sortOrder.value}&search=${search.value}`
        )
        .then((response) => {
