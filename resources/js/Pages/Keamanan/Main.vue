...
const viewDetail = (id) => {
    router.visit("/skkaad-detail", { method: "get", data: {id:id} });
};

const editForm = (id) => {
    router.visit("/skkaad-form", { method: "get", data: { id: id } });
};
    ...
    Swal({
        ...
        if (result.isConfirmed) {
            try {
                ...
                const response = await axios.post(`/api/skkaad/destroy/${id}`);
...
const fetchData = () => {
    axios
        .get(
            `/api/skkaad?page=${currentPage}&perPage=${perPage.value}&sortField=${sortField.value}&sortOrder=${sortOrder.value}&search=${search.value}`
        )
        .then((response) => {
