...
const fetchData = () => {
    if (props.id) {
        axios
            .get(`/api/skkaad/getById/${props.id}`)
            .then((res) => {
    ...
    <app-layout>
        <div class="card">
            <form @submit.prevent="submitForm(formData)">
                <h5 class="text-blue-600 uppercase">Informasi Klasifikasi Keamanan Arsip</h5>
                <hr />

                <div class="grid mb-3">
                    <div class="pb-0 col-12 md:col-12">
                        <div class="flex gap-2 flex-column">
                            <label class="font-semibold required">Klasifikasi Keamanan Arsip</label>
                            <InputText
                                v-model="formData.klas_keamanan"
                                :invalid="errors?.klas_keamanan ? true : false"
                            />
