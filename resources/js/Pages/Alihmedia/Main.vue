...
const fetchData = () => {
    axios
        .get(
            `/api/alihmedia?page=${currentPage}&perPage=${perPage.value}&sortField=${sortField.value}&sortOrder=${sortOrder.value}&search=${search.value}`
        )
        .then((response) => {
...
const exportData = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Data Arsip Antrean");

    // Ekspor file dengan timestamp
    const date = new Date().toISOString().split('T')[0];
    XLSX.writeFile(workbook, `data_arsip_antrean${date}.xlsx`);
};
